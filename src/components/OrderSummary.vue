<template>
  <div class="order-summary">
    ...
    <div class="order-summary__wrapper">
      ...
      <div
        v-if="canPlaceOrder"
        class="order-summary__payment flex flex-column"
      >
        <div class="order-summary__payment-option">
          ...
          />
          <span
            v-if="paymentMethod === 'Direct Bank Transfer'"
            class="body-small dark-gray-text order-summary__payment-option-info"
            >Make your payment directly into our bank account. Please use your Order ID as the payment reference. Your
            order will not be shipped until the funds have cleared in our account</span
          >
        </div>
        <div class="order-summary__payment-option">
          <BaseRadio
      ...
      <div
        v-if="canPlaceOrder"
        class="order-summary__place-order"
      >
        <BaseButton
          size="large"
          variant="solid-black"
          text="PLACE ORDER"
          @click.native="$emit('place-order')"
        />
      </div>
    </div>
  </div>
